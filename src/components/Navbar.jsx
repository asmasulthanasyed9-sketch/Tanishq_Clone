  ...
  return (
    <>
      ...
      <div className="promo-bar">
        ✨ Rivaah Wedding Edit — Book Your Free Bridal Styling Consultation <span>Reserve Now!</span>
      </div>
      ...
      <header className={`navbar ${scrolled ? 'scrolled' : ''}`}>
        ...
        <div className="navbar-top">
          ...
          <div className="navbar-search">
            <input type="text" placeholder="Search for bridal sets, wedding mangalsutra..." />
        ...
        <nav className="navbar-bottom">
          <ul className="nav-menu">
            {navItems.map((item, idx) => (
              ...
              >
                <span className={`nav-link ${item.highlight ? 'highlight' : ''} ${item.featured ? 'featured' : ''}`}>
                  {item.icon && (
                    <img
                      src={item.icon}
                      alt=""
                  ...
                  {item.label}
                  {item.featured && <span className="nav-badge">Rivaah</span>}
                </span>
      ...
      <aside className={`mobile-drawer ${mobileOpen ? 'open' : ''}`}>
        ...
        <div className="mobile-drawer-actions">
          <button className="nav-action-btn"><UserIcon /><span>Log in</span></button>
          <button className="nav-action-btn"><HeartIcon /><span>Wishlist</span></button>
          <a href="#" className="nav-action-btn"><StoreIcon /><span>Find Store</span></a>
        </div>
          ...
          {navItems.map((item, idx) => (
            item.noMenu || !item.tabs ? (
              <li key={idx} className="mobile-nav-item">
                <a href="#" className={item.featured ? 'featured' : ''}>
                  {item.label}
                  {item.featured && <span className="nav-badge">Rivaah</span>}
                </a>
              </li>
              ...
              <li key={idx} className="mobile-nav-item">
                <details>
                  <summary>
                    <span className={item.featured ? 'featured' : ''}>
                      {item.label}
                      {item.featured && <span className="nav-badge">Rivaah</span>}
                    </span>
                    <ChevronDownIcon />
                  ...
                  <div className="mobile-sub-links">
                    {item.tabs.map((tab, ti) => (
                      <div key={ti} className="mobile-sub-group">
                        <h6>{tab.heading}</h6>
                        <div className="mobile-sub-grid">
                          {tab.links.map((link, li) => (
                            <a href="#" key={li}>{link}</a>
                          ))}
                        </div>
                      </div>
                    ))}
